<template>
    <div>
        ...
        <div style="margin-bottom: 20px">
            ...
            <el-input style="width: 240px" placeholder="请输入用户名" v-model="params.username"></el-input>   <!--绑定变量-->
            ...
            <i class="el-icon-message" style="margin-left: 5px"></i>
            <el-input style="width: 260px;margin-left: 5px" placeholder="请输入邮箱" v-model="params.email"></el-input>
        ...
        methods: {
            changeStatus(row) {
                if (this.admin.id ===row.id && !row.status){
                    row.status = true
                    this.$notify.warning("您的操作不合法，至少保留当前管理员账号")
                    return
                }
                    ...
                    if (valid) {
                        request.put('/admin/password' , this.form).then(res => {
                            if (res.code === '200') {
                                ...
                                if (this.form.id === this.admin.id) {    //当前修改的用户id 等于当前登录的管理员id，那么修改成功之后需要重新登陆
